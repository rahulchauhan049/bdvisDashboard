...
mod_temporal_server <- function(input, output, session, dataTemporal){
  ns <- session$ns
    ...
    if (is.null(select)){
      output$pie <- renderPlotly({
        ...
        if (!nrow(dataTemporal()[-which(dataTemporal()[, input$pieselect] == ""),]) == 0) {
          dataa <- dataTemporal()[-which(dataTemporal()[, input$pieselect] == ""),]
        } else {
          dataa <- dataTemporal()
        }
      ...
      newData <- dataTemporal() %>% filter(year %in% as.numeric(select))
      output$pie <- renderPlotly({
        if (input$pieselect == "kingdom") {
          label <- ~ kingdom
        } else if (input$pieselect == "phylum") {
